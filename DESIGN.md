...
The sheet is a bottom panel in `settingsPanel`. SETTINGS and CLOSE stay on one header row. Gold section titles with a thin rule: Location, Prayer athans, Athan, Hourly athkar, Medicine reminder, Night blackout. Country and city are bordered search fields. GPS is a quiet action under location. No Default Albany button. Prayer athans has five chips (Fajr, Dhuhr, Asr, Maghrib, Isha) to mute or unmute individual athans, then a gold slider row per prayer with a percent and a PLAY preview at that volume. Athan is a compact list with a gold tick on the selected file and PLAY on the right. Athkar is On / Off, no clip demos. Subtitle is `8 AM to 10 PM · athan wins`. Medicine reminder is On / Off, Arabic / English voice, Preview, and up to six time slots with hour/minute steppers and Su–Sa day chips. Subtitle is `Spoken cue on the days and hours you pick`. Night blackout is On / Off with subtitle `11 PM to 4 AM · tap to wake`. Landscape splits location on the left and prayer athans+athan+athkar+medicine+blackout on the right. Same Inter and gold. No Material You cards. Scroll inside a column if the 7-inch cannot fit.
