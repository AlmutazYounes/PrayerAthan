...
The sheet is a bottom panel in `settingsPanel`. SETTINGS and CLOSE stay on one header row. Gold section titles with a thin rule: Location, Time adjustments, Prayer athans, Athan, Hourly athkar, Morning athkar, Medicine reminder, Night blackout. Country and city are bordered search fields. GPS is a quiet action under location. No Default Albany button. Time adjustments show the live 12-hour clock time per prayer as the main number, with a small `ISNA` / `+3 min` chip and plus/minus. Prayer athans has a mute pill on each volume row, then a gold slider and PLAY. Athan is a compact list with a gold tick on the selected file and PLAY on the right. Athkar and Night blackout use a compact gold switch on the card header. Athkar subtitle is `8 AM to 10 PM · Medicine wins that minute`. Morning athkar subtitle is `8:05 every 5 min · six clips · off by default`. Medicine reminder is a header switch, Arabic / English voice, Preview, and up to six time slots with hour/minute wheels and AM/PM plus Su–Sa day chips. Subtitle is `Spoken cue on the days and hours you pick`. Night blackout subtitle is `11 PM to 4 AM · tap to wake`. Landscape splits location+time adjustments on the left and prayer athans+athan+athkar+medicine+blackout on the right. Same Inter and gold. No Material You cards. Scroll inside a column if the 7-inch cannot fit.

### Medicine banner

When `medicinePlaying` is true, keep the stacked clocks and prayer grid. Overlay a centered bottom banner: small `MEDICINE` label in gold, primary line in the selected voice language (Arabic face for Arabic, Inter for English), secondary line in the other language smaller. Panel uses `settingsPanel` at ~92% opacity with a gold border. Tap anywhere still stops playback. Banner clears when the clip ends.
